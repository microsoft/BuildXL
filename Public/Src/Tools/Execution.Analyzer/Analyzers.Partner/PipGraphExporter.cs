...
namespace BuildXL.Execution.Analyzer
{
    ...
    {
        public Analyzer InitializePipGraphExporter()
        {
            ...
            return new PipGraphExporter(GetAnalysisInput(), jsonFilePath);
        }

        private static void WritePipGraphExporterHelp(HelpWriter writer)
        {
            writer.WriteBanner("Pip Graph Export Analysis");
    ...
    /// </summary>
    internal sealed class PipGraphExporter : Analyzer
    {
        private readonly PipGraphJsonWriter m_writer;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="PipGraphExporter"/> class
    ...
    /// </remarks>
    public sealed class PipGraphJsonWriter : IDisposable
    {
        /// <summary>
        ...
        private Dictionary<FileArtifact, int> m_fileIds = new Dictionary<FileArtifact, int>(capacity: 10 * 1000);

        /// <summary>
        /// A list of all file artifacts discovered in the graph
        /// </summary>
        public IEnumerable<FileArtifact> Artifacts { get { return m_fileIds.Keys; } }

        /// <summary>
        /// Creates a JSON writer which writes text to <paramref name="output" />
        /// </summary>
        public PipGraphJsonWriter(TextWriter output)
        {
        ...
        public PipGraphJsonWriter(string path)
        ...
        private void ExportFileDetails(IEnumerable<KeyValuePair<FileArtifact, FileContentInfo>> fileData)
        {
            ...
            foreach (var kvp in fileData)
            {
                ...
                // fileid
                int fileId;
                if (m_fileIds.TryGetValue(kvp.Key, out fileId))
                {
                    m_writer.WriteWhitespace(Environment.NewLine);
                    m_writer.WriteStartObject();
            ...
            ref int nextId)
        {
            ...
            if (!m_fileIds.ContainsKey(artifact)) // Only write if we haven't seen this artifact before (duplicates ocurr because of nested SealedDirectory content listings)
            {
                int fileId = nextId++;
                m_fileIds.Add(artifact, fileId);
                m_writer.WriteWhitespace(Environment.NewLine);
                m_writer.WriteStartObject();
        ...
        {
            int directoryId;
            if (!directoryIds.ContainsKey(artifact))
            {
                directoryId = nextId++;
            ...
            {
                int dependencyId;
                if (!m_fileIds.TryGetValue(directoryMember, out dependencyId))
                {
