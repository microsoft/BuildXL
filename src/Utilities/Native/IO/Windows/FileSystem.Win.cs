...
#pragma warning restore CA1008 // Enums should have zero value
        ...
        public unsafe bool TryPosixDelete(string pathToDelete, out OpenFileResult openFileResult)
        {
            ...
            using (handle)
            {
                ...
                if (handle.IsInvalid)
                {
                    ...
                    openFileResult = OpenFileResult.Create(hr, FileMode.Open, handleIsValid: false, openingById: false);
                    return false;
                }
                ...
                openFileResult = OpenFileResult.Create(hr, FileMode.Open, handleIsValid: true, openingById: false);
            ...
            out SafeFileHandle handle)
        {
            ...
            if (handle.IsInvalid)
            {
                ...
                var result = OpenFileResult.Create(hr, fileMode, handleIsValid: false, openingById: false);
                Contract.Assume(!result.Succeeded);
                return result;
            }
            else
            {
                var result = OpenFileResult.Create(hr, fileMode, handleIsValid: true, openingById: false);
                Contract.Assume(result.Succeeded);
                return result;
            }
        }
            ...
            out SafeFileHandle handle)
        {
            ...
            if (handle.IsInvalid)
            {
                ...
                handle = null;
                Contract.Assume(hr != 0);
                var result = OpenFileResult.Create(hr, creationDisposition, handleIsValid: false, openingById: false);
                Contract.Assume(!result.Succeeded);
                return result;
            }
            else
            {
                var result = OpenFileResult.Create(hr, creationDisposition, handleIsValid: true, openingById: false);
                Contract.Assume(result.Succeeded);
                return result;
            }
        }
        ...
        /// <inheritdoc />
        public bool TryCreateSymbolicLink(string symLinkFileName, string targetFileName, bool isTargetFile)
        {
            int res = CreateSymbolicLinkW(symLinkFileName, targetFileName, isTargetFile ? SymbolicLinkTarget.File : SymbolicLinkTarget.Directory);
            return res > 0;
        }
        ...
        private static OpenFileResult TryOpenReparsePoint(string reparsePoint, FileDesiredAccess accessMode, out SafeFileHandle reparsePointHandle)
        {
            ...
            if (reparsePointHandle.IsInvalid)
            {
                ...
                Contract.Assume(hr != 0);
                var result = OpenFileResult.Create(hr, FileMode.Open, handleIsValid: false, openingById: false);
                Contract.Assume(!result.Succeeded);
                return result;
            }
            else
            {
                var result = OpenFileResult.Create(hr, FileMode.Open, handleIsValid: true, openingById: false);
                Contract.Assume(result.Succeeded);
                return result;
            }
        }
            ...
            out SafeFileHandle handle)
        {
            ...
            if (handle.IsInvalid)
            {
                ...
                handle = null;
                Contract.Assume(hr != 0);
                var result = OpenFileResult.Create(hr, FileMode.Open, handleIsValid: false, openingById: true);
                Contract.Assume(!result.Succeeded);
                return result;
            }
            else
            {
                var result = OpenFileResult.Create(hr, FileMode.Open, handleIsValid: true, openingById: true);
                Contract.Assume(result.Succeeded);
                return result;
            }
        }
        ...
        public void GetChainOfReparsePoints(SafeFileHandle handle, string sourcePath, IList<string> chainOfReparsePoints)
        {
            ...
            do
            {
                ...
                // symlink's target might be specified using a relative path - convert to the absolute path before proceeding
                sourcePath = ConvertReparsePointTargetPathToAbsolutePath(sourcePath, possibleNextTarget.Result);
                chainOfReparsePoints.Add(sourcePath);
            ...
            Action<string, string, FileAttributes> handleEntry)
        {
            ...
            var searchDirectoryPath = Path.Combine(ToLongPathIfExceedMaxPath(directoryPath), pattern);
        ...
        {
            return EnumerateDirectoryEntries(directoryPath, recursive: false, handleEntry: (currentDirectory, fileName, fileAttributes) => handleEntry(fileName, fileAttributes));
        }
...
#pragma warning restore 0618
        ...
        private static string ToLongPathIfExceedMaxPath(string path)
        {
