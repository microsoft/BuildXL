...
namespace ContentStoreTest.Distributed.ContentLocation.NuCache
{
    ...
    {
        private static readonly SerializationPool SerializationPool = new SerializationPool();
        ...
        public static ContentLocationEntry CloneWithSpan(this ContentLocationEntry source, bool usePooledSerialization = true)
        {
            if (usePooledSerialization)
            {
                using var blob = SerializationPool.SerializePooled(source, static (ContentLocationEntry instance, ref SpanWriter writer) => instance.Serialize(ref writer));
                var blobReader = blob.WrittenSpan.AsReader();
