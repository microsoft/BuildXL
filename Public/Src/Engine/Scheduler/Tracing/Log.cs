...
namespace BuildXL.Scheduler.Tracing
{
    ...
    public abstract partial class Logger : LoggerBase
    {
        ...
        [GeneratedEvent(
            ...
            EventGenerators = EventGenerators.LocalOnly,
            Message = "{workerName} will be released because {numProcessPipsWaiting} (numProcessPipsWaiting), {estimatedSlotsForWaitingPips} (estimatedSlotsForWaitingPips) < {remainingSlots} (slots after release). Worker's Slots: {workerSlots} (total process slots), {cachelookup} (cachelookup), {execute} (execute), {ipc} (ipc).",
            EventLevel = Level.Verbose,
            EventTask = (ushort)Tasks.Distribution,
            Keywords = (int)Keywords.UserMessage)]
        public abstract void InitiateWorkerRelease(LoggingContext context, string workerName, long numProcessPipsWaiting, long estimatedSlotsForWaitingPips, int remainingSlots, int workerSlots, int cachelookup, int execute, int ipc);
