...
namespace Test.BuildXL.Processes.Detours
{
    ...
    {
        private const int NOT_ENOUGH_PRIVILEDGES_ERROR = 1314;
        private const string ExtraFileNameInDirectory = "foo.txt";

        private bool IsNotEnoughPriviledgesError(SandboxedProcessPipExecutionResult result)
        {
            if (result.Status == SandboxedProcessPipExecutionStatus.ExecutionFailed && result.ExitCode == NOT_ENOUGH_PRIVILEDGES_ERROR)
            {
            ...
            LoggingContext loggingContext = null)
        {
            ...
            string substTarget = string.Empty;
            string substSource = string.Empty;

            if (TemporaryDirectory != directoryHandlePath)
            {
                // Figure out and pass the subst strings properly.
            ...
            if (!string.IsNullOrWhiteSpace(substSource) && !string.IsNullOrWhiteSpace(substTarget))
            {
            ...
            return new SandboxedProcessPipExecutor(
                ...
                buildXlBinDirectory: binDirectory,
        ...
        public async Task CallCreateFileOnNtEscapedPath()
        {
            ...
            AssertWarningEventLogged(loggingContext, EventId.PipProcessFailedToParsePathOfFileAccess, count: 0);
        }
            ...
            List<DirectoryArtifact> outputDirectories = null)
        {
            ...
            if (createSymlink && secondFileOrDirectoryAbsolutePath.IsValid)
            {
                ...
                if (!FileUtilities.TryCreateSymbolicLink(firstFileExpandedPath, secondFileOrDirectoryExpandedPath, !isDirectoryTest))
                {
                    XAssert.Fail("Symbolic link from '{0}' to '{1}' cannot be created", firstFileExpandedPath, secondFileOrDirectoryExpandedPath);
                }
            }
        ...
        [InlineData(true)]
        [InlineData(false)]
        public async Task CallDetouredSetFileInformationFileLink(bool ignorePreloadedDlls)
        {
            var context = BuildXLContext.CreateInstanceForTesting();
            var pathTable = context.PathTable;
            ...
            using (var tempFiles = new TempFileStorage(canGetFileNames: true, rootPath: TemporaryDirectory))
            {
                ...
                if (IsNotEnoughPriviledgesError(result))
                {
                    return;
                }
        ...
        public async Task CallDetouredGetFinalPathNameByHandle(bool ignoreGetFinalPathNameByHandle)
        {
            ...
            using (var tempFiles = new TempFileStorage(canGetFileNames: true, rootPath: TemporaryDirectory))
            {
                ...
                XAssert.IsTrue(FileUtilities.TryCreateSymbolicLink(testFile, testTargetFile, true));
        ...
        public async Task TestCreateExistingDirectoryFileAccessError()
        {
            ...
            using (var tempFiles = new TempFileStorage(canGetFileNames: true, rootPath: TemporaryDirectory))
            {
                ...
                SetExpectedFailures(1, 0);
                AssertVerboseEventLogged(loggingContext, EventId.PipProcessDisallowedFileAccess);
        ...
        public async Task CallMoveSymLinkOnFilesWithGrantedAccess()
        {
            ...
            using (var tempFiles = new TempFileStorage(canGetFileNames: true, rootPath: TemporaryDirectory))
            {
                ...
                FileUtilities.TryCreateSymbolicLink(oldSymlink.ToString(pathTable), irrelevantFilePath.ToString(pathTable), true);
        ...
        public async Task TestAccessToExecutableTestNoAccess()
        {
            ...
            using (var tempFiles = new TempFileStorage(canGetFileNames: true, rootPath: TemporaryDirectory))
            {
                ...
                var pip = new Process(
                    ...
                    null,
                    null,
                    ReadOnlyArray<FileArtifact>.FromWithoutCopy(new[]
                                            {
                                                executableFileArtifact
                                            }),
                    ReadOnlyArray<FileArtifactWithAttributes>.Empty,
        ...
        public async Task CallDetouredFileCreateThatAccessesChainOfSymlinks()
        {
            ...
            using (var tempFiles = new TempFileStorage(canGetFileNames: true, rootPath: TemporaryDirectory))
            {
                ...
                FileUtilities.TryCreateSymbolicLink(intermediateSymlink.ToString(pathTable), targetFile.ToString(pathTable), true);
                FileUtilities.TryCreateSymbolicLink(sourceOfSymlink.ToString(pathTable), intermediateSymlink.ToString(pathTable), true);
        ...
        public async Task CallAccessNestedSiblingSymLinkOnFiles()
        {
            ...
            using (var tempFiles = new TempFileStorage(canGetFileNames: true, rootPath: TemporaryDirectory))
            {
                ...
                FileUtilities.TryCreateSymbolicLink("symlink.icache.link", @"..\..\targets\x64\hello.txt", true);

                Directory.SetCurrentDirectory(sourceDir.ToString(pathTable));
                FileUtilities.TryCreateSymbolicLink("symlink.imports.link", @"..\..\icache\x64\symlink.icache.link", true);
        ...
        private async Task AccessSymlinkAndVerify(BuildXLContext context, TempFileStorage tempFiles, TranslateDirectoryData translateDirectoryData, string function, AbsolutePath[] paths)
        {
            var process = CreateDetourProcess(
                context,
                context.PathTable,
            ...
            SandboxedProcessPipExecutionResult result = await RunProcessAsync(
                ...
                pip: process,
                errorString: out errorString,
                directoriesToTranslate: new List<TranslateDirectoryData> { translateDirectoryData });
        ...
        public async Task CallAccessJunctionSymlink()
        {
            ...
            using (var tempFiles = new TempFileStorage(canGetFileNames: true, rootPath: TemporaryDirectory))
            {
                ...
                FileUtilities.TryCreateSymbolicLink("symlink.link", @"..\target\hello.txt", true);
                ...
                await AccessSymlinkAndVerify(context, tempFiles, translateToReal, "CallAccessJunctionSymlink_Real",
                    new[]
                    {
                        realSymlink,
                        realTarget
                ...
                await AccessSymlinkAndVerify(context, tempFiles, translateToReal, "CallAccessJunctionSymlink_Junction",
                    new[]
                    {
                        realSymlink,
                        junctionTarget
                ...
                await AccessSymlinkAndVerify(context, tempFiles, translateToJunction, "CallAccessJunctionSymlink_Real",
                    new[]
                    {
                        junctionSymlink,
                        realTarget
                ...
                await AccessSymlinkAndVerify(context, tempFiles, translateToJunction, "CallAccessJunctionSymlink_Junction",
                    new[]
                    {
                        junctionSymlink,
                        junctionTarget
