...
const pkgContents = importFrom('Microsoft.NETFramework.ReferenceAssemblies.net462').Contents.all;

const relativeRoot = r`build/.NETFramework/v4.6.2`;

function createAssembly(binary: RelativePath) : Managed.Assembly {
    return Managed.Factory.createAssembly(pkgContents, r`${relativeRoot}/${binary}`, "net462", [], true);
}

function createFacade(binary: RelativePath) : Managed.Assembly {
    return Managed.Factory.createAssembly(pkgContents, r`${relativeRoot}/Facades/${binary}`, "net462", [], true);
}
...
export const NetFx = {
    ...
    Netstandard: {
        // This is to allow NetFX frameowrks to be mixed with net472 which does have netstandard and not requiring conditional adding the deps.
        dll: undefined,
    },
    Microsoft: {
        Activities: {
