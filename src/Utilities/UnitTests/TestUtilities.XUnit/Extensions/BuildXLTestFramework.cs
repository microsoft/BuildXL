...
{
    /// <summary>
    /// Custom Xunit test framework that calls <see cref="BuildXLTestFrameworkExecutor"/>.
    /// </summary>
    [SuppressMessage("Microsoft.Performance", "CA1812:AvoidUninstantiatedInternalClasses", Justification = "Used by Xunit")]
    internal class BuildXLTestFramework : XunitTestFramework
    {
        private readonly Logger m_logger;

        /// <nodoc />
        public BuildXLTestFramework(IMessageSink messageSink)
            : base(messageSink)
        {
            m_logger = new Logger();
        ...
        protected override ITestFrameworkExecutor CreateExecutor(AssemblyName assemblyName)
            => new BuildXLTestFrameworkExecutor(m_logger, assemblyName, SourceInformationProvider, DiagnosticMessageSink);
    }
}
