...
static uint64_t ProcessTreeResidentSize(pid_t pid, const rlim_t max_proc_count, bool includeChildren, bool *success)
{
        ...
        if (includeChildren)
        {
            ...
            *success &= child_count >= 0;
            ...
            for (int i = 0; (i < child_count) && *success; i++)
            {
                ...
                memory_usage += ProcessTreeResidentSize(child_pid, max_proc_count, includeChildren, success);
            }
        }
    }
...
int GetPeakWorkingSetSize(pid_t pid, uint64_t *buffer, bool includeChildren)
{
    ...
    // We look at the resident size because we care about physical memory consumption and
    ...
    uint64_t mem_usage = ProcessTreeResidentSize(pid, rl.rlim_cur, includeChildren, &success);
    if (!success)
    {
        return RUNTIME_ERROR;
    }
