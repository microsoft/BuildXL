...
namespace BuildXL.Native.Processes.Unix
{
    ...
    {
        private static readonly Lazy<bool> m_isDebugModeEnabled = new Lazy<bool>(() => 
        {
            bool result = true;
            unsafe
            {
                Sandbox.CheckForDebugMode(&result);
            }
            return result;
        });
        ...
        public unsafe int NormalizeAndHashPath(string path, out byte[] normalizedPathBytes)
        {
            ...
            fixed (byte* resultBuffer = normalizedPathBytes)
            {
                return Sandbox.ExternNormalizeAndHashPath(pPath, resultBuffer, normalizedPathBytes.Length);
            }
        }
        ...
        /// <inheritdoc />
        public bool IsNativeInDebugConfiguration() => m_isDebugModeEnabled.Value;
