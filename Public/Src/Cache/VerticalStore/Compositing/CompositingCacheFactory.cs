...
namespace BuildXL.Cache.Compositing
{
    ...
    public sealed class CompositingCacheFactory : ICacheFactory
    {
        ...
        public async Task<Possible<ICache, Failure>> InitializeCacheAsync(ICacheConfigData cacheData, Guid activityId, IConfiguration configuration = null, BuildXLContext buildXLContext = null)
        {
            ...
            var maybeCache = await CacheFactory.InitializeCacheAsync(compositingConfig.MetadataCache, activityId, configuration, buildXLContext);
            ...
            maybeCache = await CacheFactory.InitializeCacheAsync(compositingConfig.CasCache, activityId, configuration, buildXLContext);
            if (!maybeCache.Succeeded)
            {
                Analysis.IgnoreResult(await metadata.ShutdownAsync(), justification: "Okay to ignore shutdown status");
                return maybeCache.Failure;
