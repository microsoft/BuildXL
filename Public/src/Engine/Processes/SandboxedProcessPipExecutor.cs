...
namespace BuildXL.Processes
{
    ...
    public sealed class SandboxedProcessPipExecutor : ISandboxedProcessFileStorage
    {
        ...
        /// </summary>
        private const int OutputChunkInLines = 10000;
        ...
        private readonly AbsolutePath m_buildXLBinDirectory;
        ...
        public SandboxedProcessPipExecutor(
            ...
            AbsolutePath buildXlBinDirectory = default,
            ...
            ITempDirectoryCleaner tempDirectoryCleaner = null)
        {
            ...
            m_fileAccessManifest =
                new FileAccessManifest(m_pathTable, directoryTranslator)
                {
                    ...
                    CrashOnCBSSOOMError = sandBoxConfig.CrashOnCBSSOOMError,
                    PipId = m_pip.SemiStableHash,
            ...
            m_buildXLBinDirectory = buildXlBinDirectory;
        ...
        /// </summary>
        public async Task<SandboxedProcessPipExecutionResult> RunAsync(CancellationToken cancellationToken = default, ISandboxedKextConnection sandboxedKextConnection = null)
        {
            try
            {
            ...
            HashSet<AbsolutePath> allInputPathsUnderSharedOpaques)
        {
            ...
            else
            {
                ...
                if (exitedSuccessfullyAndGracefully)
                {
                    ...
                    // On failure, logging should have happened as part of the move operation
        ...
        private bool PrepareFileAccessMonitoringCommon(LoggingContext loggingContext)
        {
            ...
            if (m_buildXLBinDirectory.IsValid)
            {
                m_fileAccessManifest.AddScope(
                    m_buildXLBinDirectory,
                    FileAccessPolicy.MaskNothing,
                    FileAccessPolicy.AllowReadAlways);
            }
            ...
            // a non deterministic behavior. When AllowDirectoryCreationUnderWritableMount is set to false, we will not downgrade
            // the CreateDirectory to a read-only probe in Detours and will emit file access error/warning for all CreateDirectory
            // call that do not have an explicit allowing policy in the manifest, regardless whether the corresponding directory exists or not.
            ...
            // always exist, allowing such requests would not lead to any changes on the disk. Moreover, we applying an exact path policy (i.e., not a scope policy).
            ...
            if (allowInternalErrorsLogging || checkMessageCount)
            {
                ...
                // Delete the file
                FileUtilities.TryDeleteFile(m_detoursFailuresFile, tempDirectoryCleaner: m_tempDirectoryCleaner);
        ...
        {
            string toolName = m_pip.GetToolName(m_pathTable).ToString(m_pathTable.StringTable);
            if (string.Compare(toolName, "csc", StringComparison.OrdinalIgnoreCase) == 0 ||
                string.Compare(toolName, "csc.exe", StringComparison.OrdinalIgnoreCase) == 0)
            {
                return ProcessKind.Csc;
            }

            if (string.Compare(toolName, "cvtres", StringComparison.OrdinalIgnoreCase) == 0 ||
                string.Compare(toolName, "cvtress.exe", StringComparison.OrdinalIgnoreCase) == 0)
            {
                return ProcessKind.Cvtres;
            }

            if (string.Compare(toolName, "reson", StringComparison.OrdinalIgnoreCase) == 0 ||
                string.Compare(toolName, "resonexe.exe", StringComparison.OrdinalIgnoreCase) == 0)
            {
                return ProcessKind.Resonexe;
            }

            if (string.Compare(toolName, "rc", StringComparison.OrdinalIgnoreCase) == 0 ||
                string.Compare(toolName, "rc.exe", StringComparison.OrdinalIgnoreCase) == 0)
            {
                return ProcessKind.RC;
            }

            if (string.Compare(toolName, "cccheck", StringComparison.OrdinalIgnoreCase) == 0 ||
                string.Compare(toolName, "cccheck.exe", StringComparison.OrdinalIgnoreCase) == 0)
            {
                return ProcessKind.CCCheck;
            }

            if (string.Compare(toolName, "ccdocgen", StringComparison.OrdinalIgnoreCase) == 0 ||
                string.Compare(toolName, "ccdocgen.exe", StringComparison.OrdinalIgnoreCase) == 0)
            {
                return ProcessKind.CCDocGen;
            }

            if (string.Compare(toolName, "ccrefgen", StringComparison.OrdinalIgnoreCase) == 0 ||
                string.Compare(toolName, "ccrefgen.exe", StringComparison.OrdinalIgnoreCase) == 0)
            {
                return ProcessKind.CCRefGen;
            }

            if (string.Compare(toolName, "ccrewrite", StringComparison.OrdinalIgnoreCase) == 0 ||
                string.Compare(toolName, "ccrewrite.exe", StringComparison.OrdinalIgnoreCase) == 0)
            {
                return ProcessKind.CCRewrite;
            }

            if (string.Compare(toolName, "windbg", StringComparison.OrdinalIgnoreCase) == 0 ||
                string.Compare(toolName, "windbg.exe", StringComparison.OrdinalIgnoreCase) == 0)
            {
                return ProcessKind.WinDbg;
            }

            if (string.Compare(toolName, "tool.xamlcompilerwrapper", StringComparison.OrdinalIgnoreCase) == 0 ||
                string.Compare(toolName, "tool.xamlcompilerwrapper.exe", StringComparison.OrdinalIgnoreCase) == 0)
            {
                return ProcessKind.XAMLWrapper;
            }
        ...
        private static bool StringLooksLikeRCTempFile(string fileName)
        {
            ...
            if (c3.ToUpperInvariantFast() != 'C' && c3.ToUpperInvariantFast() != 'D')
            {
                return false;
            }
        ...
        /// using white lists.
        /// </remarks>
        private bool GetSpecialCaseRulesForSpecialTools(
            AbsolutePath fileAccessPath)
        {
            ...
            {
                case ProcessKind.Csc:
                case ProcessKind.Cvtres:
                case ProcessKind.Resonexe:
                ...
                case ProcessKind.CCCheck:
                case ProcessKind.CCDocGen:
                case ProcessKind.CCRefGen:
                case ProcessKind.CCRewrite:
                    ...
                    {
                        // WriteWarningF(L"Access to the pdb file '%s' is granted.", absolutePath);
                        return true;
                    }

                    break;

                case ProcessKind.WinDbg:
                case ProcessKind.Other:
                case ProcessKind.Unknown:
            ...
            out IReadOnlyDictionary<AbsolutePath, IReadOnlyCollection<AbsolutePath>> sharedDynamicDirectoryWriteAccesses)
        {
            ...
            using (PooledObjectWrapper<Dictionary<AbsolutePath, CompactSet<ReportedFileAccess>>> accessesByPathWrapper = ProcessPools.ReportedFileAccessesByPathPool.GetInstance())
            {
                ...
                foreach (ReportedFileAccess reported in result.ExplicitlyReportedFileAccesses)
                {
                    ...
                    accessesByPath[parsedPath] = existingAccessesToPath.Add(reported);
                }
                    ...
                    ProcessPools.AccessUnsorted.GetInstance())
                {
                    ...
                    // These special tools/cases should be whitelisted, but we already have customers deployed specs without
                    // using white lists.
                    List<AbsolutePath> excluded = new List<AbsolutePath>();
                    foreach (var reportedPathAndAccesses in accessesByPath)
                    {
                        AbsolutePath reportedPath = reportedPathAndAccesses.Key;

                        if (GetSpecialCaseRulesForSpecialTools(reportedPath) ||
                            GetSpecialCaseRulesForCoverageAndSpecialDevices(reportedPath))
                        {
                            excluded.Add(reportedPath);
                        }
                    }

                    foreach (AbsolutePath excludedPath in excluded)
                    {
                        accessesByPath.Remove(excludedPath);
                    }

                    // Remove all the special file accesses that need removal.
                    RemoveInjectableFileAccesses(accessesByPath);
        ...
        private void RemoveInjectableFileAccesses(Dictionary<AbsolutePath, CompactSet<ReportedFileAccess>> accessesByPath)
        {
        ...
        private void HandleErrorsFromTool(string error)
        {
            ...
            var process = GetProcessKind();

            if (process == ProcessKind.Csc)
            {
                // BUG 1124595
        ...
        private async Task<string> ReadNextChunk(TextReader reader, SandboxedProcessOutput output, Predicate<string> filterPredicate = null)
        {
            try
            {
                ...
                {
                    StringBuilder sb = wrapper.Instance;
                    for (int i = 0; i < OutputChunkInLines; i++)
                    {
                        string line = await reader.ReadLineAsync();
                        if (line == null)
                        {
        ...
        private int ReportSurvivingChildProcesses(SandboxedProcessResult result)
        {
            ...
            var unexpectedSurvivingChildProcesses = result
                ...
                .Where(pr =>
                    !PathEndsWith(pr, "ProcessTreeContextCreator.exe") &&
                    !m_pip.AllowedSurvivingChildProcessNames.Any(procName => PathEndsWith(pr, procName.ToString(m_context.StringTable))));

            int numErrors = 0;
            foreach (ReportedProcess p in unexpectedSurvivingChildProcesses)
            {
                numErrors++;
                Tracing.Logger.Log.PipProcessChildrenSurvivedError(m_loggingContext, m_pip.SemiStableHash, m_pip.GetDescription(m_context), p.Path);

                if (PathEndsWith(p, "mspdbsrv.exe"))
                {
                    Tracing.Logger.Log.PipProcessMspdbsrv(m_loggingContext, m_pip.SemiStableHash, m_pip.GetDescription(m_context));
                }
            }

            return numErrors;

            bool PathEndsWith(ReportedProcess pr, string suffix)
            {
                return pr.Path.EndsWith(suffix, StringComparison.OrdinalIgnoreCase);
            }
        }
