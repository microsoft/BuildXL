...
namespace BuildXL.Native.IO.Windows
{
    ...
    public sealed class FileUtilitiesWin : IFileUtilities
    {
            ...
            ITempDirectoryCleaner tempDirectoryCleaner = null)
        {
            ...
            {
                // Maintain a list of files that we delete for diagnostics
                HashSet<string> deletedPaths = stringSetPool.Instance;
                ...
                if (!success)
                {
                    ...
                    throw new BuildXLException(FileUtilitiesMessages.DeleteDirectoryContentsFailed + directoryPath +
                        " Directory contents: " + Environment.NewLine + FindAllOpenHandlesInDirectory(directoryPath, deletedPaths));
                }
        ...
        private void DeleteEmptyDirectoryHelper(string directoryPath, ITempDirectoryCleaner tempDirectoryCleaner)
        {
            ...
            if (!SkipPosixDelete)
            {
                ...
                if (s_fileSystem.TryPosixDelete(directoryPath, out result))
                {
                    return;
                }
            }
        ...
        private bool DeleteFileInternal(string path, ITempDirectoryCleaner tempDirectoryCleaner, out OpenFileResult deleteResult)
        {
            ...
            if (!SkipPosixDelete)
            {
                // Trying Posix delete semantics first. Will only work on win10 rs2+.
                // We do not log this failure since it will be frequent and expected on many OSes
            ...
            if (deleteResult.Status == OpenFileStatus.AccessDenied || deleteResult.Status == OpenFileStatus.SharingViolation)
            {
                ...
                if (!string.IsNullOrWhiteSpace(deletionTempDirectory) && !path.Contains(deletionTempDirectory))
                {
                    ...
                    {
                        Tracing.Logger.Log.FileUtilitiesDiagnostic(context, path, "Successfully move-deleted file.");
                        return true;
                    }
                }
            }
        ...
        private static bool TrySetWriteAttributesPermission(string path, AccessControlType control)
        {
            ...
            {
                Logger.Log.FileUtilitiesDiagnostic(Events.StaticContext, path, "Failed to set WriteAttributes permission: " + ex.Message);
                return false;
            }
        ...
        private static bool TryTakeOwnershipAndSetAcl(string path)
        {
            ...
            takeOwn.WaitForExit(60 * 1000);

            // It is possible that TakeOwn fails, but ICACLS will still succeed, so do not return false on failure
            if (takeOwn.ExitCode != 0)
            {
                Logger.Log.SettingOwnershipAndAclFailed(
                    Events.StaticContext,
                    path,
                    takeOwn.StartInfo.FileName,
                    takeOwn.StartInfo.Arguments,
                    takeOwn.StandardError.ReadToEnd() + takeOwn.StandardOutput.ReadToEnd());
            }
            ...
            var icacls = Process.Start(new ProcessStartInfo()
            {
                ...
                RedirectStandardOutput = true,
                FileName = "icacls",
                Arguments = I($"\"{path}\" /grant {Environment.UserName}:(F)")
            });
            ...
            if (icacls.ExitCode != 0)
            {
                Logger.Log.SettingOwnershipAndAclFailed(
                    Events.StaticContext,
                    path,
                    icacls.StartInfo.FileName,
                    icacls.StartInfo.Arguments,
                    icacls.StandardError.ReadToEnd() + icacls.StandardOutput.ReadToEnd());
                return false;
            }
        ...
        public Possible<string> GetFileName(string path)
        {
            try
            {
                return FileSystemWin.GetFileName(path);
            }
        ...
        public string GetKnownFolderPath(Guid knownFolder)
        {
            ...
            int hr = NativeMethods.SHGetKnownFolderPath(ref knownFolder, 0, IntPtr.Zero, out pathPtr);
            if (hr == 0)
            {
        ...
        public string GetUserSettingsFolder(string appName)
        {
            ...
            {
                throw new BuildXLException("Missing environment variable 'LOCALAPPDATA'.");
            }
#else
            var homeFolder = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData);
#endif
        ...
        public bool HasWritableAccessControl(string path)
        {
            ...
            return true;
#else
            // OSX will never run this code, but it does need to compile it.
