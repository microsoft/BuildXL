...
const assertion1 = Contract.assert(typeof(firstOutput) === 'SharedOpaqueDirectory');")
        ...
        [Fact]
        public void ExportedValuesCanBeConsumedWithNonEmptyQualifier()
        {
            // By setting a non-empty default qualifier for the JavaScript resolver, we ensure the top-level evaluation path would use a non-empty
            // qualifier. On the other hand, the expression referencing the export in the sibling DScript module will naturally get coerced to the empty
            // qualifier (since JS export expressions are generated by the JS resolver in a qualifier-agnostic fashion). This test ensures that the resolver
            // can handle this scenario and the exported values can be consumed successfully in DScript.
            var config =
                Build(
                    rushExports: "[{symbolName: 'exportSymbol', content: ['@ms/project-A']}]",
                    moduleName: "rushTest",
                    addDScriptResolver: true,
                    defaultQualifier: "{configuration: 'debug'}")
               .AddJavaScriptProject("@ms/project-A", "src/A")
               .AddSpec("module.config.dsc", "module({name: 'dscriptTest', nameResolutionSemantics: NameResolutionSemantics.implicitProjectReferences});")
               // Consume 'exportSymbol' from DScript and make sure it is an array of shared opaques.
               .AddSpec(@"
...
const assertion2 = Contract.assert(firstOutput.root.isWithin(d`src/A`));")
            ...
            Assert.True(result.IsSuccess);
            // Make sure all exports are found
            AssertInformationalEventLogged(LogEventId.RequestedExportIsNotPresent, 0);
        }
