...
declare function toolOption(opt: any);
...
interface Object {
    ...
    /** A way to set the custom merge function. Similar to Array.add to handle BuildXL's imutability, this returns the new object.*/
...
namespace Number {
    export declare function parseInt(val: string, radix?: number): number;
}
...
interface Array<T> {
	...
	/**
     * Returns an array identical to this array, but with a custom merge function that appends to returned array when merging
     * Note: append is the default merging mechanism for arrays
     */
    appendWhenMerged() : Array<T>;

    /**
     * Returns an array identical to this array, but with a custom merge function that prepends to the returned array when merging
     */
    prependWhenMerged() : Array<T>;

    /**
     * Returns an array identical to this array, but with a custom merge function that replaces the returned array when merging
     */
...
namespace Array {
    /** Returns an integer array of increasing elements from lo to hi (including both lo and hi). If lo > hi the function returns the empty list. */
...
declare function package(packageConfig: any): void;
declare function module(moduleConfig: any): void;
declare function glob(root: string, pattern: string, depth?: number): any[];
declare function globR(root: string, pattern: string, depth?: number): any[];
declare function addIf<T>(condition: boolean, ...items: T[]) : T[];
declare function addIfLazy<T>(condition: boolean, items: () => T[]) : T[];
...
declare function __d(format: string, ...args: any[]): any;

/** Creates a file artifact (doesn't create a physical file in the file system). */
declare function __f(format: string, ...args: any[]): any;

interface Qualifier {
}

declare function declareQualifierSpace(x: any): any;

declare let qualifier: any;
declare function withQualifier(q: any): any;
...
// Just a namespace for testing, not part of the real prelude
namespace TestPrelude {
	export interface TestType { _testTypeBranding: string };
...
type DoubleWritePolicy =  
        ...
        // win the race. Consider this will result in a non-deterministic deployment for a given build, and therefore unsage.
