...
namespace BuildXL.Utilities
{
    ...
    public sealed class PerformanceCollector : IDisposable
    {
        ...
        {
            double? procUsage = null;

            // Processor time consumed by this process
            if (m_processTimeLastCollectedAt == DateTime.MinValue)
            {
                m_processTimeLastCollectedAt = DateTime.UtcNow;
                m_processTimeLastValue = currentProcess.TotalProcessorTime;
            }
            else
            {
                ...
                procUsage = (100.0 * (processTimeCurrentValue - m_processTimeLastValue).Ticks) /
        ...
        private double? GetMachineCpuMacOS()
        {
            ...
            unsafe
            {
                ...
                {
                    fixed(CpuLoadInfo *ptr = &m_lastCpuLoadInfo)
                    {
                        GetCpuLoadInfo(ptr);
                    }
                }

                CpuLoadInfo buffer = new CpuLoadInfo();
                if (GetCpuLoadInfo(&buffer) == MACOS_INTEROP_SUCCESS)
                {
        ...
        public sealed class Aggregator : IDisposable
        {
            ...
            public MachinePerfInfo ComputeMachinePerfInfo(bool ensureSample = false)
            {
                ...
                using (var sbPool2 = Pools.GetStringBuilder())
                {
                    ...
                    logFileSummary.AppendFormat(" BuildXLCPU:{0}%", perfInfo.ProcessCpuPercentage);

                    perfInfo.ProcessWorkingSetMB = SafeConvert.ToInt32(ProcessWorkingSetMB.Latest);
                    logFileSummary.AppendFormat(" BuildXLRAM:{0}MB", perfInfo.ProcessWorkingSetMB);
