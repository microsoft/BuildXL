...
/// </summary>
/// <remarks>
/// The debug parameter is temporary to catch the non-deterministic bug
/// </remarks>
    ...
    __out FileAccessPolicy& policy)
{
    ...
    case ProcessKind::Resonexe:
        ...
        if (HasSuffix(absolutePath, absolutePathLength, L".tmp")) {
            // WriteWarningF(L"Access to the temporary file '%s' is granted.", absolutePath);
#if SUPER_VERBOSE
...
ProcessKind  g_ProcessKind = ProcessKind::Other;
...
void InitProcessKind()
{
    ...
    const struct ProcessPair pairs[] = {
            { L"windbg.exe", ProcessKind::WinDbg },
            { L"rc.exe", ProcessKind::RC },
            { L"ccrewrite.exe", ProcessKind::CCRewrite },
            { L"cccheck.exe", ProcessKind::CCCheck },
            { L"ccrefgen.exe", ProcessKind::CCRefGen },
            { L"ccdocgen.exe", ProcessKind::CCDocGen },
            { L"csc.exe", ProcessKind::Csc },
            { L"cvtres.exe", ProcessKind::Cvtres },
            { L"resonexe.exe", ProcessKind::Resonexe} };
