        ...
        protected async Task RunTestAsync(OperationContext context, Func<IBlobCacheTopology, ICacheSession, BlobNamespaceId, IBlobCacheAccountSecretsProvider, Task> run)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    {
                        await session!.ShutdownAsync(context).IgnoreFailure();
                    }
                }
                finally
                {
                    await cache.ShutdownAsync(context).IgnoreFailure();
                }
            }
            finally
            {
                // Kill the Azurite process immediately rather than returning it to the pool,
                // to prevent blob.exe from surviving the test pip's sandbox.
                process.Dispose(close: true);
            }
        }
    }
}
