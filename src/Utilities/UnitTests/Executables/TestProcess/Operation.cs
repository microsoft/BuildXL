...
{
    /// <summary>
    /// Defines allowable process operation and their behaviiors
    /// </summary>
    public sealed class Operation
    {
        ...
        private void DoCreateSymlink()
        {
            ...
            if (!FileUtilities.TryCreateSymbolicLink(linkPath, target, SymLinkFlag == SymbolicLinkFlag.FILE))
            {
                int error = Marshal.GetLastWin32Error();
                throw new InvalidOperationException("Symbolic link creation from " + LinkPath.Path.ToString(PathTable) + " to '" + target + "' failed with error " + error + ".");
            }
        }

        private void DoCreateHardlink()
        {
            if (!ExternWinCreateHardLink(FileOrDirectoryToString(LinkPath), FileOrDirectoryToString(Path), IntPtr.Zero))
            {
                int error = Marshal.GetLastWin32Error();
                throw new InvalidOperationException("Hard link creation from " + LinkPath.Path.ToString(PathTable) + " to " + Path.Path.ToString(PathTable) + " failed with error " + error + ".");
            }
        }
