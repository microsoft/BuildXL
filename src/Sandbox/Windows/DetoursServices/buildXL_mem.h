...
// This file defines a memory interface for Detours (hense the dd_ prefix).
// The general allocation APIs are stubbed out and one should call only the dd_* methods.
// The memory allocation dome from the Detours library happens on a private heap.
...
inline void _dd_alligned_free(void *alignedMemory)
{
