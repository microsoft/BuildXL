...
#define NT_PATH_SEPARATOR       L'\\'
#define UNIX_PATH_SEPARATOR     L'/'
#define PATH_DOT                L'.'
#define PATH_DRIVE_SEPARATOR    L':'
...
/// IsPathSeparator
///
/// Checks whether the given character is a path separator (checking against all platforms).
/// Both platforms' path separators are invalid characters in the other system's paths.
inline bool IsPathSeparator(PathChar c)
{
    return c == NT_PATH_SEPARATOR || c == UNIX_PATH_SEPARATOR;
}
...
{
    if (path[0] != 0 && IsDriveLetter(path[0]) && path[1] == L':' && IsPathSeparator(path[2]))
    {
        return true;
    }
    return false;
