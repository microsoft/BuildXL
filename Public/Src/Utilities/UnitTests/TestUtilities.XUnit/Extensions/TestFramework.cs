...
namespace Test.BuildXL.TestUtilities.XUnit.Extensions
{
    ...
    internal class TestFramework : XunitTestFramework
    {
            ...
            : base(messageSink)
        {
            ...
            TaskScheduler.UnobservedTaskException += (sender, args) => m_logger.LogError("Task unobserved exception: " + args.Exception?.DemystifyToString());
            ...
            AppDomain.CurrentDomain.UnhandledException += (sender, args) => m_logger.LogError("Domain unhandled error: " + (args.ExceptionObject as Exception)?.DemystifyToString());
