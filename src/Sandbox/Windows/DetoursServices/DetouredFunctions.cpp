    ...
    if (errorCode == ERROR_SUCCESS)
    {
        ...
        if (reparsePointType == IO_REPARSE_TAG_SYMLINK)
        {
            ...
            else
            {
                ...
                wstring srcPath = path.GetPathString();
...
{
    result.assign(fileName, (WCHAR) fileNameLength / sizeof(WCHAR));
    DWORD lastError = GetLastError();
...
NTSTATUS HandleFileLinkInformation(
    ...
    _In_  FILE_INFORMATION_CLASS FileInformationClass)
{
    assert((FILE_INFORMATION_CLASS_EXTRA)FileInformationClass == FILE_INFORMATION_CLASS_EXTRA::FileLinkInformation);
    ...
    _In_  FILE_INFORMATION_CLASS FileInformationClass)
{
    ...
    switch (fileInformationClassExtra)
    {
        ...
        case FILE_INFORMATION_CLASS_EXTRA::FileLinkInformation:
            ...
            {
                return HandleFileLinkInformation(FileHandle, IoStatusBlock, FileInformation, Length, FileInformationClass);
            }
            break;
...
static void ReportProcessDataDetoursHeapBeforeCrash()
{
    if (!ShouldCrashOnCBSSOOMError())
    {
        return;
    }
    ...
    if (isCBSSProcess)
    {
        // Try to get the module name to send a ProcessData message. This way we get a chance to see what the Detours 
        // Max Heap Size was before crashing. We just care about the size of the Detours Heap. The process name will be the process that crashes.
        FILETIME ft;
        ft.dwLowDateTime = 0;
        ft.dwHighDateTime = 0;
        ...
        // Wait for 5 secs to allow the message to make it to BuildXL
        Sleep(5000);
...
IMPLEMENTED(Detoured_RtlFreeHeap)
BOOLEAN NTAPI Detoured_RtlFreeHeap(
    _In_     PVOID HeapHandle,
    _In_opt_ ULONG Flags,
    _In_     PVOID HeapBase)
{
    return Real_RtlFreeHeap(
        HeapHandle,
        Flags,
        HeapBase);
}

IMPLEMENTED(Detoured_RtlAllocateHeap)
PVOID NTAPI Detoured_RtlAllocateHeap(
    _In_     PVOID  HeapHandle,
    _In_opt_ ULONG  Flags,
    _In_     SIZE_T Size)
{
    PVOID ret = Real_RtlAllocateHeap(
        HeapHandle,
        Flags,
        Size);

    if (isCBSSProcess && ret == nullptr)
    {
        ReportProcessDataDetoursHeapBeforeCrash();
    }

    return ret;
}

IMPLEMENTED(Detoured_RtlReAllocateHeap)
PVOID NTAPI Detoured_RtlReAllocateHeap(
    ...
    _In_     SIZE_T Size)
{
    PVOID ret = Real_RtlReAllocateHeap(
        HeapHandle,
        Flags,
        BaseAddress,
        Size);
...
IMPLEMENTED(Detoured_VirtualAlloc)
PVOID WINAPI Detoured_VirtualAlloc(
    _In_opt_ LPVOID lpAddress,
    _In_     SIZE_T dwSize,
    _In_     DWORD  flAllocationType,
    _In_     DWORD  flProtect)
{
    PVOID ret = Real_VirtualAlloc(
        lpAddress,
        dwSize,
        flAllocationType,
        flProtect);
...
// New disposition flags introduced in WIN10_rs1 NOT DOCUMANTED FOR NOW. WINDOWS WILL DOCUMANT THEM!!!
...
typedef enum _FILE_INFO_BY_HANDLE_CLASS_EX {
    FileDispositionInfoEx = 0x15,
    FileRenameInfoEx = 0x16
} FILE_INFO_BY_HANDLE_CLASS_EX, *PFILE_INFO_BY_HANDLE_CLASS_EX;
    ...
    _In_ const wstring&       fullPath)
{
    ...
    targetFileName.assign(pRenameInfo->FileName, (WCHAR)pRenameInfo->FileNameLength / sizeof(WCHAR));
    ...
    _In_ DWORD                     dwBufferSize)
{
    bool isDisposition =
        ...
        || FileInformationClass == FILE_INFO_BY_HANDLE_CLASS_EX::FileDispositionInfoEx;

    bool isRename =
        FileInformationClass == FILE_INFO_BY_HANDLE_CLASS::FileRenameInfo
        || FileInformationClass == FILE_INFO_BY_HANDLE_CLASS_EX::FileRenameInfoEx;
    ...
    if (isDisposition) 
    {
        ...
        else if (FileInformationClass == FILE_INFO_BY_HANDLE_CLASS_EX::FileDispositionInfoEx) 
        {
