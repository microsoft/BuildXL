...
namespace BuildXL.Utilities.Authentication
{
    ...
    public class VssCredentialsFactory
    {
        ...
        public async Task<VssCredentials> CreateVssCredentialsWithAadAsync(Uri baseUri)
        {
            ...
            // Since BuildXL uses a server process that may launch the process (or this library is used by a service pip) that calls this function,
            // it is not easy to determine its parent console process
            ...
            foreach (var provider in providers)
            {
                ...
                {
                    if (provider is MsalDeviceCodeTokenProvider)
                    {
                        // Using device code requires displaying a message on the console
                        // This library is used by cache and service pips which are not set up to display messages directly to the console, therefore we skip this.
                        m_logger.LogInformation($"Skipping unsupported provider '{provider.Name}'");
                        continue;
                    }
