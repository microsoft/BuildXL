...
echo .
	...
	call %~dp0\..\..\bxl.cmd /q:Release /q:Debug /q:DebugDotNetCore /q:ReleaseDotNetCore /exp:incrementalscheduling- /cacheGraph- /forcePopulatePackageCache -All -SharedCacheMode ConsumeAndPublish /phase:Schedule
	if %ERRORLEVEL% NEQ 0 (
		echo. 1>&2
		echo --------------------------------------------------------------- 1>&2
	...
	call %~dp0\..\..\bxl.cmd /q:Release /q:Debug /q:DebugDotNetCore /q:ReleaseDotNetCore /exp:incrementalscheduling- -All -SharedCacheMode ConsumeAndPublish /logsDirectory:Out\Logs\PopulateSharedCache
	if %ERRORLEVEL% NEQ 0 (
		echo. 1>&2
		echo --------------------------------------------------------------- 1>&2
		echo - BuildXL failed to populate shared cache:  1>&2
