...
| GraphSemistableFingerprint | Overrides the graph semistable fingerprint computed from the pip graph. The supplied value is deterministically hashed into a fingerprint. |
...
| LogToKustoIngestUri | Kusto cluster URI and database for direct queued ingestion of unstructured telemetry. Expected format: https://<cluster>.<region>.kusto.windows.net/<database>?clientId=<clientId>&tenantId=<tenantId>&serviceConnectionId=<serviceConnectionId> |
...
| PipFirstReportActivityTimeout | How long to wait for the first file access report before warning a process may be stalled. Allows a longer startup window for slow-initializing processes. When not set, falls back to pipReportActivityTimeout. Windows only. |
...
| PipReportActivityTimeout | How long to wait between consecutive file access reports before warning a process may be stalled. The argument allows an expression that represents a time duration, like "3s", "500ms", "30m", "1.5h". Disabled by default. Windows only. |
...
| PipUsageMLMode | Controls Pip Usage ML evaluation based on per-pip history and historical-table availability. Defaults to Disabled generally and HistoricDataUnavailable in CloudBuild. Requires an internal build with compatible model resources. |
...
| SkipScrubbingOnCleanMachine | When enabled, skips scrubbing if no prior BuildXL execution has occurred on this machine. Enabled by default in ADO pipelines. Defaults to off. |
...
| Unsafe_IgnoreUntrackedPathsInFullReparsePointResolving | When enabled, untracked paths, if symlinks, are not fully resolved. Defaults to off. |
| Unsafe_MonitorCreateProcessAsUser | Control whether detours on Windows will monitor CreateProcessAsUser[A|W]. Enabled by default. |
...
| Unsafe_TreatStatAsProbe | Control whether on Linux stat calls are treated as probes. Enabled by default. The correct behavior is to treat stat calls as reads. The unsafe option is for now the default behavior for compatibility reasons. |
