...
// Is this the CBSS process
bool isCBSSProcess = false;
...
static void SetEventLogSource(const std::wstring& a_name)
{
    ...
    if (ERROR_SUCCESS == last_error)
    {
        ...
        const char* DetoursServices = "DetoursServices.dll";

        last_error = RegSetValueEx(key,
            L"EventMessageFile",
            0,
            REG_SZ,
            (BYTE*)DetoursServices,
            sizeof(DetoursServices));
...
struct ProcessCreationAttributes {
	...
	ProcessCreationAttributes() = default;
...
static CreateDetouredProcessStatus CreateProcessAttributes(
	...
	processCreationAttributes = ProcessCreationAttributes();
...
*/
#ifdef DETOURS_SERVICES_NATIVES_LIBRARY
// TODO: Investigation of CBSS crash. Remove when VSO WI 1080854 is resolved.
static void SetIsCBSSProcess()
{
    isCBSSProcess = false;

    wchar_t procName[3 * MAX_PATH]; // Just allocate on the heap a big amont of chars, so we don't do dynamic allocation.
    ZeroMemory(procName, sizeof(wchar_t) * (3 * MAX_PATH));

    DWORD numChars = GetModuleFileNameW(NULL, procName, _countof(procName));

    // If we fail to get the proc name, just continue.
    if (numChars == 0)
    {
        return;
    }

    // If the length of the file name is not proper, just continue.

    // First check with no extension.
    size_t strLen = wcslen(procName);
    if (strLen < 5)
    {
        // Not our string. Return.
        return;
    }

    // Make it lowercase.
    for (size_t i = 0; i < strLen; i++) {
        procName[i] = towlower(procName[i]);
    }

    // If the cbss or cbss.exe is the process name, then we crash. Otherwise we don't crash.
    if (procName[strLen - 5] == L'\\' &&
        procName[strLen - 4] == L'c' &&
        procName[strLen - 3] == L'b' &&
        procName[strLen - 2] == L's' &&
        procName[strLen - 1] == L's' &&
        procName[strLen] == L'\0')
    {
        isCBSSProcess = true;
    }
    ...
    if (!isCBSSProcess)
    {
        ...
        if (procName[strLen - 9] == L'\\' &&
            procName[strLen - 8] == L'c' &&
            procName[strLen - 7] == L'b' &&
            procName[strLen - 6] == L's' &&
            procName[strLen - 5] == L's' &&
            procName[strLen - 4] == L'.' &&
            procName[strLen - 3] == L'e' &&
            procName[strLen - 2] == L'x' &&
            procName[strLen - 1] == L'e' &&
            procName[strLen] == L'\0')
        {
            isCBSSProcess = true;
        }
...
static bool DllProcessAttach()
{
    ...
    // TODO: Investigation of CBSS crash. Remove when VSO WI 1080854 is resolved.
    // Sets if this is the cbss process.
    SetIsCBSSProcess();
...
// end #define ATTACH
    ...
    // We can't do this earlier because the flags are not initialized till the parsing of the manifest.
    ...
    if (!DisableDetours())
    {
        ...
        if (GetProcessKind() != ProcessKind::WinDbg) {
            ATTACH(CreateFileW);
            ATTACH(CreateFileA);
       
            if (ShouldLogProcessData() && isCBSSProcess)
            {
                ATTACH(RtlFreeHeap);
                ATTACH(RtlAllocateHeap);
                ATTACH(RtlReAllocateHeap);
                ATTACH(VirtualAlloc);
            }
