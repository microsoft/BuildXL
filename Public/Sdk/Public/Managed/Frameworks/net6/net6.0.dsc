...
const windowsRuntimeFiles = [
    ...importFrom("Microsoft.NETCore.App.Runtime.win-x64.6.0").Contents.all.getContent().filter(f => f.extension === a`.dll` && !ignoredAssembly(f)),
...
const osxRuntimeFiles = [
    ...importFrom("Microsoft.NETCore.App.Runtime.osx-x64.6.0").Contents.all.getContent().filter(f => macOSRuntimeExtensions(f) && !ignoredAssembly(f)),
    ...importFrom("runtime.osx-x64.Microsoft.NETCore.DotNetHostResolver.6.0").Contents.all.getContent().filter(f => macOSRuntimeExtensions(f)),
    ...importFrom("runtime.osx-x64.Microsoft.NETCore.DotNetHostPolicy.6.0").Contents.all.getContent().filter(f => macOSRuntimeExtensions(f)),
];

const linuxRuntimeFiles = [
    ...importFrom("Microsoft.NETCore.App.Runtime.linux-x64.6.0").Contents.all.getContent().filter(f => linuxRuntimeExtensions(f) && !ignoredAssembly(f)),
    ...importFrom("runtime.linux-x64.Microsoft.NETCore.DotNetHostResolver.6.0").Contents.all.getContent().filter(f => linuxRuntimeExtensions(f)),
    ...importFrom("runtime.linux-x64.Microsoft.NETCore.DotNetHostPolicy.6.0").Contents.all.getContent().filter(f => linuxRuntimeExtensions(f)),
];
...
function createDefaultAssemblies() : Shared.Assembly[] {
    ...
    return dlls.map(file  => Shared.Factory.createAssembly(pkgContents, file, "net6.0", [], true));
}
