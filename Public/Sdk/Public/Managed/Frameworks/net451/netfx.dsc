...
const pkgContents = importFrom('Microsoft.NETFramework.ReferenceAssemblies.net451').Contents.all;

const relativeRoot = r`build/.NETFramework/v4.5.1`;

function createAssembly(binary: RelativePath) : Managed.Assembly {
    return Managed.Factory.createAssembly(pkgContents, r`${relativeRoot}/${binary}`, "net451", [], true);
}

function createFacade(binary: RelativePath) : Managed.Assembly {
    return Managed.Factory.createAssembly(pkgContents, r`${relativeRoot}/Facades/${binary}`, "net451", [], true);
}
