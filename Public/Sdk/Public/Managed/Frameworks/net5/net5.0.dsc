...
export declare const qualifier: {targetFramework: "net5.0"};
...
const windowsRuntimeFiles = [
    ...importFrom("Microsoft.NETCore.App.Runtime.win-x64.5.0").Contents.all.getContent().filter(f => f.extension === a`.dll` && !ignoredAssembly(f)),
    ...importFrom("runtime.win-x64.Microsoft.NETCore.DotNetHostResolver.5.0").Contents.all.getContent().filter(f => f.extension === a`.dll`),
    ...importFrom("runtime.win-x64.Microsoft.NETCore.DotNetHostPolicy.5.0").Contents.all.getContent().filter(f => f.extension === a`.dll`),
];

const osxRuntimeFiles = [
    ...importFrom("Microsoft.NETCore.App.Runtime.osx-x64.5.0").Contents.all.getContent().filter(f => macOSRuntimeExtensions(f) && !ignoredAssembly(f)),
    ...importFrom("runtime.osx-x64.Microsoft.NETCore.DotNetHostResolver.5.0").Contents.all.getContent().filter(f => macOSRuntimeExtensions(f)),
    ...importFrom("runtime.osx-x64.Microsoft.NETCore.DotNetHostPolicy.5.0").Contents.all.getContent().filter(f => macOSRuntimeExtensions(f)),
];

const linuxRuntimeFiles = [
    ...importFrom("Microsoft.NETCore.App.Runtime.linux-x64.5.0").Contents.all.getContent().filter(f => linuxRuntimeExtensions(f) && !ignoredAssembly(f)),
    ...importFrom("runtime.linux-x64.Microsoft.NETCore.DotNetHostResolver.5.0").Contents.all.getContent().filter(f => linuxRuntimeExtensions(f)),
    ...importFrom("runtime.linux-x64.Microsoft.NETCore.DotNetHostPolicy.5.0").Contents.all.getContent().filter(f => linuxRuntimeExtensions(f)),
];
...
export function crossgenProvider(runtimeVersion: Shared.RuntimeVersion): Shared.CrossgenFiles {
    ...
    {
        case "osx-x64":
            const osxFiles = importFrom("Microsoft.NETCore.App.Runtime.osx-x64.5.0").Contents.all;
            return { 
                crossgenExe: osxFiles.getFile(r`tools/crossgen`),
                JITPath: osxFiles.getFile(r`runtimes/osx-x64/native/libclrjit.dylib`)
            };
        case "win-x64":
            const winFiles = importFrom("Microsoft.NETCore.App.Runtime.win-x64.5.0").Contents.all;
            return {
...
export const framework : Shared.Framework = {
    ...
    supportedRuntimeVersion: "v5.0",
    assemblyInfoTargetFramework: ".NETCoreApp,Version=v5.0",
...
function createDefaultAssemblies() : Shared.Assembly[] {
    const pkgContents = importFrom("Microsoft.NETCore.App.Ref50").Contents.all;
    ...
    return dlls.map(file  => Shared.Factory.createAssembly(pkgContents, file, "net5.0", [], true));
}
